...
def generate_rust_trace(csv_file_path, output_file_path):
    with open(csv_file_path, 'r') as csv_file:
        ...
        for row in reader:
            ...
            unofficial = row['unofficial opcode']
            ...
            unofficial_op = '*' if unofficial == 'yes' else ' '
            if mode == 'IMM':
                ...
                use_stored = mnemonic not in ['JMP', 'JSR']
                rust_mode = '&AddressingMode::Absolute'
                addr_prefix = '_'
                stored_prefix = '' if use_stored else '_'

                if use_stored:
                    operands = 'format!("${:04X} = {:02X}", mem_addr, stored_value)'
                else:
                    operands = 'format!("${:04X}", mem_addr)'
            elif mode == 'ABSX':
                rust_mode = '&AddressingMode::AbsoluteX'
                addr_prefix = '_'
                operands = 'format!("${:04X},X @ {:04X} = {:02X}", (mem_addr.wrapping_sub(self.registers.x as u16)), mem_addr, stored_value)'
            elif mode == 'ABSY':
                rust_mode = '&AddressingMode::AbsoluteY'
                addr_prefix = '_'
                operands = 'format!("${:04X},Y @ {:04X} = {:02X}", (mem_addr.wrapping_sub(self.registers.y as u16)), mem_addr, stored_value)'
            elif mode == 'IND':
                rust_mode = '&AddressingMode::Indirect'
                addr_prefix = '_'
                stored_prefix = '_'
                operands = 'format!("(${:04X}) = {:04X}", self.mem_read_u16(self.pc.wrapping_add(1)), mem_addr)'
            elif mode == 'INDX':
                ...
                operands = (
                    ...
                    'address, (mem_addr.wrapping_sub(self.registers.y as u16)), mem_addr, stored_value)'
                )
            elif mode == 'IMP':
            ...
            0x{opcode[2:]} => {{ // {mnemonic} {mode}
                ...
                format!(
                    "{{:04X}}  {{:<9}}{unofficial_op}{{:<31}} A:{{:02X}} X:{{:02X}} Y:{{:02X}} P:{{:02X}} SP:{{:02X}}", 
                    self.pc,
